...
describe('base-agents.md', () => {
  ...
  it('documents the execution environment and writable mounts', () => {
    ...
    expect(baseAgents).toContain('HA API')
    expect(baseAgents).toContain('I run inside HAOS as an ingress add-on')
  })
  ...
  it('explains the agent boundaries and first-person self-description behavior', () => {
    ...
    expect(baseAgents).toContain('persistent `/data` files survive restarts')
  })
})
