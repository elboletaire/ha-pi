...
export function registerChannelTool(pi: ExtensionAPI, registry: ChannelRegistry): void {
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: StringEnum(['send', 'send_file', 'list', 'test'] as const, { description: 'Action to perform' }) as any,
      ...
      caption: Type.Optional(Type.String({ description: 'Caption text for the file (optional, for send_file)' })),
    }) as any,
    ...
    async execute(_toolCallId, _params) {
      ...
      switch (params.action) {
        ...
        case 'send_file': {
          ...
          const r = await registry.sendFile(params.adapter, params.recipient ?? '', params.file_path, params.caption)
          result = r.ok
