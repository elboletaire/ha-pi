...
export class ChatBridge {
  ...
  private async runWithRpc(
    ...
    try {
      const { session: rpcSession, wasRestarted, reason } = await this.rpcManager!.getSessionWithStatus(senderKey)

      // Set up streaming draft callback if enabled and adapter supports it
      let onStreaming: ((text: string) => void) | undefined
      if (this.config.streamingDrafts && adapter?.sendDraft) {
  ...
  private createSession(message: IncomingMessage): SenderSession {
    return {
      adapter: message.adapter,
      sender: message.sender,
      displayName: (message.metadata?.firstName as string) || (message.metadata?.username as string) || message.sender,
      queue: [],
      processing: false,
