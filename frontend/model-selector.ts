...
export function sortAndFilterModels(
  ...
  const filtered = models.filter((model) => {
    ...
    const haystack = [model.provider, model.id, model.name, modelKey(model)].join(' ').toLowerCase()
...
function syncSelectionToCurrent() {
  ...
  const currentIndex = currentModelKey ? filtered.findIndex((model) => modelKey(model) === currentModelKey) : -1
